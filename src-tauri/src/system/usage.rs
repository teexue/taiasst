                ...
                for i in 0..device_count {
                    if let Ok(device) = nvml.device_by_index(i) {
                        let mut gpu = GpuUsage {
                            name: device.name().unwrap_or_else(|_| format!("GPU {}", i)).to_string(),
                            utilization: None,
                            memory_total: None,
                            memory_used: None,
                            temperature: None,
...
/// 这个函数可以直接从前端调用
#[tauri::command]
pub fn get_system_metrics() -> SystemUsage {
...
/// Tauri命令：获取CPU使用情况
#[tauri::command]
pub fn get_cpu_metrics() -> CpuUsage {
    let mut sys = init_system_monitor();
...
/// Tauri命令：获取内存使用情况
#[tauri::command]
pub fn get_memory_metrics() -> MemoryUsage {
    let mut sys = init_system_monitor();
...
/// Tauri命令：获取磁盘使用情况
#[tauri::command]
pub fn get_disk_metrics() -> Vec<DiskUsage> {
    let mut sys = init_system_monitor();
...
/// Tauri命令：获取GPU使用情况
#[tauri::command]
pub fn get_gpu_metrics() -> Vec<GpuUsage> {
    get_gpu_usage()
}
